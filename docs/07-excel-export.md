...
| フォーマット | 状態 |
|---|---|
| 既存Excel互換 (`legacy.xlsx`) | ✅ 実装済 |
| CSV 出力 | ✅ 実装済 |
| シンプル一覧 (`simple.xlsx`) | ⛔ **未実装** (`docs/08-roadmap.md` で deferred) |

> シンプル一覧は当面 CSV で代替する方針。賞与計算用の集計が必要になったタイミングで
> 改めて実装可否を判断する。仕様 (列定義など) は下記に残してあるが、現状コードはない。

## 2つのフォーマット (仕様)

将来的にユーザーはExcel出力時にフォーマットを選べるようにする予定だった：

1. **既存Excel互換（legacy）**: 既存invoice-toolが出力していた `集計用.xlsx` の構造を再現 (実装済)
2. **シンプル一覧（simple）**: フラットなテーブル形式（賞与計算しやすい） **(未実装)**
...
## フォーマット2: シンプル一覧（simple） — **未実装**

> 以下は当初仕様。現状 CSV 出力で代替しているため、xlsx 版は未実装。
> 実装するかは利用者からの要望次第で再判断する。

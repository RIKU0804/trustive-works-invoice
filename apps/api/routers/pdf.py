...
# X-Organization-Id は呼び出し側 (Next.js サーバ) で UUID をセットしている。
# 形式の最低限のバリデーション (ハイフン区切りの 8-4-4-4-12 hex)。
_UUID_RE = re.compile(
...
def _build_response(contents: bytes, org_id: str) -> ParseResponse:
    """同期の重い処理 (PDF パース + AI 分類)。

    呼び出し元の async エンドポイントから run_in_threadpool 経由で実行し、
    イベントループをブロックしないようにする。

    org_id は AI キャッシュキーに含めることでテナント間のキャッシュ漏れを防ぐ。
    """
    try:
        ...
        try:
            ...
            raise
        except (pdfplumber.pdfminer.pdfparser.PDFSyntaxError, OSError, ValueError) as exc:
        ...
        refined_rows, ai_records = classify_low_confidence_rows(
            classified_rows, org_id=org_id,
        )
        ...
        # 期待された 4xx/5xx はそのまま伝播 (ログ汚染しない)
        raise
    except Exception:
        # 想定外の 5xx は必ずスタックトレースを残す
        logger.exception("PDF parse pipeline failed")
        raise
...
async def parse_pdf(
    ...
    # この API 自体はキー↔org のマッピングを持たないため、形式 (UUID) のみ検証する。
    ...
    return await run_in_threadpool(_build_response, contents, org_id)
